...
class CrawlCSV(APIView):  # Extend from the view you need
    ...
    @staticmethod
    def crawl_wrapper(consumer_key, consumer_secret, query, count):
        api = authenticate(consumer_key=consumer_key, consumer_secret=consumer_secret)
        if not api:
            return Response(CrawlCSV.__name__ + '. Not able to connect to twitter API.', HTTP_400_BAD_REQUEST)
        else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not consumer_key or not consumer_secret or not query or not count:
            return Response(self.__name__ + '. Missing parameter.', HTTP_400_BAD_REQUEST)
